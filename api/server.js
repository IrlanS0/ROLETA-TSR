...
io.on('connection', (socket) => {
    ...
    socket.on('create_room', (data) => {
        ...
        const game = (data.game || 'fc26').toString().trim().substring(0, 40); // [NOVO] Jogo escolhido pelo Host

        rooms[roomId] = {
            password: data.password || '',
            hostId: socket.id,
            hostNickname: hostNickname, // Apelido do Host
            game: game,            // [NOVO] Define de qual base de times esta sala sorteia
        ...
        socket.join(roomId);
        console.log(`[SALA] ${socket.id} CRIOU a sala ${roomId} (Apelido: ${hostNickname}, Jogo: ${game})`);
        socket.emit('room_created', { roomId: roomId, game: game }); // [NOVO] Confirma o jogo para o Host
    });
    ...
    socket.on('join_room', (data) => {
        ...
        socket.emit('room_joined', { 
            ...
            game: room.game || 'fc26',       // [NOVO] Jogo definido pelo Host desta sala
